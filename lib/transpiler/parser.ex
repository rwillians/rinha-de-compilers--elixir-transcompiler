defmodule Transpiler.Parser do
  ...
  @doc """
  A function that, given a parser, can parse the `value` into a `Transpiler.Parser.Module`.
  """
  @spec parse(parser :: module, value :: term) ::
    {:ok, [Transpiler.Parser.Function.Definition.t()], Transpiler.Parser.Module.t()} | {:error, term}

  def parse(parser, value) do
    with {:ok, tree} <- apply(parser, :parse, [%Transpiler.Parser.Module{}, value]),
  ...
  """
  @spec parse(parser :: module, value :: term) ::
    {[Transpiler.Parser.Function.Definition.t()], Transpiler.Parser.Module.t()}
  ...
  defp split_fns_from_tree(%Transpiler.Parser.Module{} = tree), do: {extract_fns(tree), exclude_fns(tree)}

  defp extract_fns(acc \\ [], node)
  defp extract_fns(acc, %Transpiler.Parser.Module{block: next}), do: extract_fns(acc, next)
  defp extract_fns(acc, %Transpiler.Parser.Function.Definition{} = node), do: extract_fns([%{node | next: nil} | acc], node.next)
  defp extract_fns(acc, %_{next: next}), do: extract_fns(acc, next)
  defp extract_fns(acc, _), do: :lists.reverse(acc)

  defp exclude_fns(%Transpiler.Parser.Module{block: next} = node), do: %{node | block: exclude_fns(next)}
  defp exclude_fns(%Transpiler.Parser.Function.Definition{} = node), do: exclude_fns(node.next)
